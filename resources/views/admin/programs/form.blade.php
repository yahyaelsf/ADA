<form action="{{ route('admin.programs.store') }}" method="post">
    @csrf
    @isset($program)
        <input type="hidden" name="pk_i_id" value="{{ $program->getKey() }}"/>
    @endisset

    <x-localizable-inputs :inputs="$inputs" :item="$program">
            ...
            <div class="col-lg-12 col-xl-12">
                <div class="kt-avatar kt-avatar--outline" id="kt_user_avatar_1">
                    <div class="kt-avatar__holder"
                         style="background-image: url({{ $program && $program->s_cover ?  asset($program->s_cover) : '' }})"></div>
